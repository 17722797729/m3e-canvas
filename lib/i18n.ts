...
export const UI = {
  ...
  gachaTen: { ja: "まとめて引くボタン", en: "The many-at-once button", zh: "连抽按钮文字" },
  manyCount: { ja: "まとめて引く回数", en: "How many at once", zh: "连抽次数" },
  prizeHint: {
  ...
  showCellNames: { ja: "アイテム名を表示", en: "Show item names", zh: "显示物品名" },
  cellItemName: { ja: "アイテム名", en: "Item name", zh: "物品名" },
  cellNameHint: {
    ja: "中身のあるスロットの下にこの名前が出ます（空のスロットには出ません）。名前のぶん行の間隔が広がるので、下の行とぶつかりません。",
    en: "This name sits under every slot that holds something; empty slots stay bare. The rows move apart by the name's own height, so it never touches the row below.",
    zh: "有内容的格子下面显示这段文字（空格子不显示）。行距会自动加上文字的高度，所以不会和下一行挤在一起。",
  },
  gridCount: {
    ja: "{c} 列 × {r} 行・{n} セル",
    en: "{c} × {r} · {n} cells",
  ...
  renameComposite: { ja: "組み合わせの名前を変更", en: "Rename the composite", zh: "重命名组合" },
...
export const KO: Record<UIKey, string> = {
  ...
  composites: "조합 부품", composite: "조합", compositeNameHint: "조합 이름을 입력하세요", overwriteName: "같은 이름이 있습니다. 덮어쓸까요?", renameComposite: "조합 이름 변경", deleteComposite: "조합 삭제",
...
export const KIND_TEXT: Record<
  Lang,
  Record<string, { noun: string; label?: string; label2?: string; supporting?: string }>
> = {
  ja: {
    box: { noun: "ボックス" },
    ...
    slot: { noun: "スロット", label: "スタート" },
    calendar: { noun: "カレンダー", label: "チェックイン" },
    moneyTree: { noun: "金のなる木", label: "ゆする", label2: "{n}回ゆする" },
    eggSmash: { noun: "金の卵", label: "たたく" },
  ...
  en: {
    ...
    slot: { noun: "slot machine", label: "Spin" },
    calendar: { noun: "check-in calendar", label: "Check in" },
    moneyTree: { noun: "money tree", label: "Shake", label2: "Shake {n} times" },
    eggSmash: { noun: "golden eggs", label: "Smash" },
  ...
  zh: {
    ...
    slot: { noun: "老虎机", label: "开始" },
    calendar: { noun: "日历签到", label: "签到" },
    moneyTree: { noun: "摇钱树", label: "摇一摇", label2: "摇 {n} 下" },
    eggSmash: { noun: "欢乐砸蛋", label: "砸金蛋" },
  ...
  ko: {
    ...
    slot: { noun: "슬롯 머신", label: "시작" },
    calendar: { noun: "출석 달력", label: "출석" },
    moneyTree: { noun: "돈나무", label: "흔들기", label2: "{n}번 흔들기" },
    eggSmash: { noun: "황금알", label: "깨기" },
